...
<footer class="text-center" id="footer" data-aos="fade-up" data-aos-easing="ease-in-out" data-aos-duration="500">
    <div class="container mt-3">
        <div class="copyright">
            &copy; Copyright <strong><span> Sekolah Clarissa</span></strong>. All Rights Reserved
        </div>
    </div>
</footer><!-- End Footer -->
<script>
    ...
    buttonCek.addEventListener("click", function cek(e) {
        ...
        var nilaiInggris = document.getElementById("nilaiInggris").value
        var nilaiIpa = document.getElementById("nilaiIpa").value
        var nilaiIps = document.getElementById("nilaiIps").value
        var nilaiAkhir = [nilaiMatematika, nilaiIndonesia, nilaiInggris, nilaiIpa, nilaiIps]
        console.log(nilaiAkhir)

        var total = 0;
        for (var i = 0; i < nilaiAkhir.length; i++) {
            total += parseFloat(nilaiAkhir[i])
            var hasil = total / nilaiAkhir.length;
        }
        document.getElementById("demo").value = hasil.toFixed(2)
        console.log("nilai, Kumulatif " + hasil.toFixed(2))
...
<<script src="<?php echo base_url('assets/admin') ?>/js/fullcalendar.min.js">
    </script>
